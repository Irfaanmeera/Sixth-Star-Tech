...
export default async function handler(req, res) {
  ...
  try {
    const messageHtml = `
          ...
          <p>Other Services: ${data.otherservice}</p>
    `;

    await transporter.sendMail({
      ...mailOptions,
      subject: 'SEO Enquiry Form Submission',
      html: messageHtml,
    });

    // Insert data into the database
    const query = `
      INSERT INTO seo_form_submissions ( name, phone, companyname, companyurl, companyemail, service, otherservice, checkbox, pageUrl, consent,  submission_date	
)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )
    `;
    const values = [
