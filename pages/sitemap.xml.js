...
export const getServerSideProps = async ({ res }) => {
  const baseUrl = process.env.BASE_URL || 'https://sixthstartech.com';
  const pagesDirectory = path.join(process.cwd(), 'pages');

  // Directories to exclude from the sitemap
  const excludeDirectories = ['api', 'blogs', 'forms', 'public', 'section'];
  ...
  const getAllPages = (directory) => {
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        ...
        if (['/_app.jsx', '/_document.jsx', '/_error.jsx'].includes(`/${file}`)) continue;
